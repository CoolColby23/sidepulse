...
def shared_secret_from_env() -> str:
    return os.environ.get("SIDE_SHARED_SECRET") or os.environ.get("PIXIE_SHARED_SECRET", "")


def resolve_device_token(value: Any) -> str | None:
    if isinstance(value, str) and value.strip():
        return value.strip()
    return os.environ.get("SIDE_DEVICE_TOKEN") or os.environ.get("PIXIE_DEVICE_TOKEN")
...
def first_led_text(envelope: dict[str, Any]) -> str | None:
    for key in ("leds", "LEDS.txt", "LEDS.TXT", "text"):
...
def index_html() -> str:
    default_token = html.escape(os.environ.get("SIDE_DEVICE_TOKEN") or os.environ.get("PIXIE_DEVICE_TOKEN", ""))
...
<body>
  ...
  <label>Optional raw LEDS.TXT</label>
...
def main() -> int:
    host = os.environ.get("SIDE_SERVER_HOST") or os.environ.get("PIXIE_SERVER_HOST", "127.0.0.1")
    port = int(os.environ.get("SIDE_SERVER_PORT") or os.environ.get("PIXIE_SERVER_PORT", str(DEFAULT_PORT)))
    print(f"Serving SidePulse push server at http://{host}:{port}")
    uvicorn.run("server:app", host=host, port=port, log_level=os.environ.get("SIDE_LOG_LEVEL", "info"))
    return 0
