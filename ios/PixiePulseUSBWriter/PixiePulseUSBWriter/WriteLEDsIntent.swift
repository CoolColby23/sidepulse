...
struct WriteLEDsIntent: AppIntent {
    static var title: LocalizedStringResource = "Write SidePulse LEDS.TXT"
    static var description = IntentDescription("Writes the supplied LED program to LEDS.TXT on the selected USB drive.")
    static var openAppWhenRun = false

    @Parameter(title: "LEDS.TXT Text")
    var ledsText: String
    ...
    func perform() async throws -> some IntentResult {
        ...
        await MainActor.run {
            AppModel.shared.recordWriteSuccess("Shortcut wrote LEDS.TXT")
        }
        return .result()
    }
}
