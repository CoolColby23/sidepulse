...
def normalize_payload(event_name: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if event_name == "stop" and isinstance(payload.get("status"), str):
        normalized["message"] = f"Cursor stopped: {payload['status']}"
    return normalized
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        normalized = normalize_payload(args.event, raw if isinstance(raw, dict) else {})
        provider, log_path, line = routed_hook_payload(
            "cursor", args.log.expanduser(), json.dumps(normalized, separators=(",", ":"))
        )
