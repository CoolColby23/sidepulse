...
def add_sidepulse_battery_parser(subparsers: argparse._SubParsersAction) -> None:
    battery = subparsers.add_parser(
        "battery",
        help="Show or mirror Mac battery state to SidePulse/PulseDot LEDs.",
    )
...
def build_parser(prog: str = "agent-monitor") -> argparse.ArgumentParser:
    ...
    leds = subparsers.add_parser("leds", help="Mirror aggregate agent status to SidePulse/PulseDot LEDs.")
    ...
    install = subparsers.add_parser("install", help="Install Codex and/or Claude monitor hooks.")
    install.add_argument("provider", choices=("all", "codex", "claude"), nargs="?", default="all")
    ...
    uninstall = subparsers.add_parser("uninstall", help="Remove Codex and/or Claude monitor hooks.")
    uninstall.add_argument("provider", choices=("all", "codex", "claude"), nargs="?", default="all")
    ...
    hook_log.add_argument("--provider", choices=("codex", "claude"), required=True)
...
def cmd_doctor(args: argparse.Namespace) -> int:
    configs = [detect_codex_config(), detect_claude_config()]
...
def monitor_from_args(args: argparse.Namespace) -> AgentMonitor:
    if args.codex_log or args.claude_log:
        fallback_sources = default_sources()
        sources = []
        if args.codex_log:
            sources.append(SourceSpec("codex", args.codex_log.expanduser()))
        else:
            sources.extend(source for source in fallback_sources if source.provider == "codex")
        if args.claude_log:
            sources.append(SourceSpec("claude", args.claude_log.expanduser()))
        else:
            sources.extend(source for source in fallback_sources if source.provider == "claude")
    else:
...
def render_watch_dashboard(
    ...
    fixed_width = 9 + 22 + 20 + 8 + 18 + 16 + 18
    cwd_width = max(18, table_width - fixed_width)
    widths = [9, 22, 20, 8, 18, 16, cwd_width]
    headers = ["Provider", "Agent", "Mode", "Age", "Event", "Tool", "Cwd"]
    ...
    for status in statuses:
        ...
        lines.append(table_row(row, widths, mode_index=2, mode=status.mode, color=color))
    lines.append(table_separator(widths))
    return "\n".join(lines)
...
def describe_status(status: AgentStatus, now) -> str:
    ...
    return (
        ...
        f" event={status.event_name}{tool} age={age}s{stale}{cwd}"
    )
